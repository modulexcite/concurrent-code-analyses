...
namespace ForRefactoring
{
    class CodeAction : ICodeAction
    {
        ...
        private ForStatementSyntax loop;

        
        public CodeAction(IDocument document, ForStatementSyntax loop)
        {
        ...
        {
            get { return "Replace with Parallel.For for readability"; }
        }

        public CodeActionEdit GetEdit(CancellationToken cancellationToken)
        {
            
            var initText = loop.Declaration.ToString();
            string[] words = initText.Split(' ');
            var initValue = words[words.Length - 1];
            var variableName = words[words.Length - 3];

            var conditional = loop.Condition.ToString();
            words = conditional.Split(' ');
            var bound = words[words.Length - 1];
            
            var function = loop.DescendantNodes().OfType<ParenthesizedLambdaExpressionSyntax>().First().AddParameterListParameters(new[] { Syntax.Parameter(Syntax.Identifier(variableName)) });

            function = updateLambdaFunction(function);
   
            string arguments = string.Format("({0}, {1}, {2})", initValue, bound, function.ToFullString());

            var newParallelFor= Syntax.ExpressionStatement( Syntax.InvocationExpression(
                    Syntax.ParseExpression("Parallel.For"),
            ...
            var annotated = new SyntaxAnnotation();
            newParallelFor= newParallelFor.WithAdditionalAnnotations(annotated);
            ...
            //Debug.WriteLine("heyoooo"+Helper.getNextNode(newRoot.DescendantNodes().Where(a=> a.HasAnnotation(annotated)).First()).ToString());

            newRoot = newRoot.RemoveNode(Helper.getNextNode(newRoot.DescendantNodes().Where(a => a.HasAnnotation(annotated)).First()) as SyntaxNode, SyntaxRemoveOptions.KeepExteriorTrivia);
        ...
        {
            var id= loop.Declaration.Variables.First().Identifier;
